...
def contact(request):
    if request.method == "POST":
        ...
        invalid_imput = ["", " "]
        if (
            name in invalid_imput
            or email in invalid_imput
            or phone in invalid_imput
            or message in invalid_imput
        ):
...
def search(request):
    ...
    if len(results) == 0:
        ...
        message = ""
    return render(
        request, "search.html", {"results": results, "query": query, "message": message}
    )
